...
public class DijkstraTest {
    ...
    int[] dijkstra(int start, List<int[]>[] graph) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] nextNode : graph[currentNodeId]) {
                int nextNodeId = nextNode[0];
                // todo start到 currentNodeId的最短距离 + currentNode到nextNode的距离.  与start到 nextNode的最短距离进行比较.
                //int distanceToNextNode = distanceTo[currentNodeId] +  weight(currentNodeId, nextNodeId);
                int distanceToNextNode = distanceTo[currentNodeId] + nextNode[1];
